# -*- coding: utf-8 -*-
"""
Created on Tue Oct 11 10:25:59 2016
...
class ParseHtml:
    def __init__(self, url_file, outfile, url_unparse_file):
        ...
        self._head_p = '//header[@id="title"]/h1'
        self._description_p = '//div[@class="description"]'
        self._step_p = '//*[@class="section"]'
    ...
    def ParseStep(self, fw):
        ...
        for stepEle in StepCont:
            div_id = stepEle.attrib['id']
            if not div_id.startswith("section"):
                continue
            stepTextNode = stepEle.xpath('./p')
            stepDetail = stepTextNode[0].text
            if stepDetail.strip('\n').strip() != "":
                answer_res += self._sep + stepDetail + '\n'
        return answer_res + '\n'
...
if __name__ == "__main__":
    ph = ParseHtml("C:/Code/data/smallbusinessChron.Guide.cb64.txt", "C:/Code/data/smallbusinessChronParseResult.tsv", "C:/Code/data/smallbusinessChronUnableParseUrls.tsv")
    ph.Parse()
