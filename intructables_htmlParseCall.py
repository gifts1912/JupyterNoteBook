...
class ParseHtml:
    def __init__(self, url_file, outfile, url_unparse_file):
        ...
        self._description_p = '/html/body/div[2]/div/div[2]/div[3]/div[1]'
        self._step_p = '//h2[@class="step-title"]'
        self._sep = "*"
    ...
    def ParseDescription(self):
       ...
       # print (des[0].atrrib)
        return des[0].attrib['content'].strip("&nbsp;")
    ...
    def ParseStep(self, fw):
        ...
        for ele in StepCont:#step1, step2
            ...
            fw.write(self._sep + ele.text + '\n')
            
            #Sub steps of Step1
            subPath = '../div[@class="step-body"]/descendant::text()'   
            subSteps = ele.xpath(subPath)
            subStepMerge = ' '.join(subSteps)
            subStepMerge = subStepMerge.replace('\n', ' ')
            subStepMerge = re.sub(r'\s{2,}', ' ', subStepMerge)
            fw.write(self._sep * 2 + subStepMerge + '\n')
            '''
...
if __name__ == "__main__":
    ph = ParseHtml("C:/Code/data/instructables.Guide.cb64.txt", "C:/Code/data/instructablesParseResult.tsv", "C:/Code/data/instructablesUnableParseUrls.tsv")
    ph.Parse()
